(function () {
  ...
  async function refreshRadar() { try { const response = await fetch('/api/hyperliquid/radar/status?_ts='+Date.now(),{cache:'no-store'}); const data = await response.json(); if (!response.ok) throw Error(data.error || `HTTP ${response.status}`); renderRadar(data); } catch (error) { if ($('#hlRadarState')) $('#hlRadarState').textContent = 'Радар недоступен: ' + error.message; } }
