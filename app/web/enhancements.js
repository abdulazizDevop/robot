(function () {
  ...
  function renderRadar(data) {
    ...
    const staleCount = Number(data.stale_count || 0); const staleMins = Number(data.stale_oldest_minutes || 0); const emptyActivity = rows.length ? '' : (   !data.running ? 'Радар остановлен — нажмите «Старт радара», иначе живых адресов не будет.'   : scanning ? 'Сканирование live продолжается — ждём первые ответы API, старые данные не подставляются.'   : staleCount ? `Живых адресов нет. ${staleCount} найденных ранее скрыты как устаревшие (последнее подтверждение ${staleMins > 1440 ? Math.round(staleMins/1440)+' дн.' : Math.round(staleMins/60)+' ч.'} назад) — они не показываются как живые.`   : 'Радар работает, но ни один адрес пока не прошёл фильтры. Ослабьте пороги в настройках.');
